...
fn test_pretty_print_minified() {
    let minified = r#"(function(){var a=1,b=2;function add(x,y){return x+y;}console.log(add(a,b));})();"#;
    let result = parser::pretty_print(minified, "test.js", false);
    assert!(result.is_ok(), "Parser should succeed on valid JS");
    let code = result.unwrap();
    assert!(code.lines().count() > 3, "Pretty-print should expand minified code");
    assert!(code.contains("function add"), "Should preserve function declaration");
}
...
fn test_detect_react() {
    ...
    assert!(names.contains(&"React"), "Should detect React: got {names:?}");
}
...
fn test_detect_tailwind() {
    ...
    let libs = lib_detect::detect_libraries(source, false);
    let names: Vec<_> = libs.iter().map(|l| l.name.as_str()).collect();
    assert!(names.contains(&"TailwindCSS"), "Should detect Tailwind: got {names:?}");
}
...
fn test_no_false_positives_on_plain_js() {
    ...
    let libs = lib_detect::detect_libraries(source, false);
    assert!(libs.is_empty(), "Plain JS should have no library detections");
}
...
fn test_fetch_local_rollup_fixture() {
    ...
    let f = result.unwrap();
    assert!(f.source.contains("use strict"), "Fixture should contain 'use strict'");
}
...
fn test_fetch_local_webpack_fixture() {
    ...
    let f = result.unwrap();
    assert!(f.source.contains("__webpack_require__"), "Webpack fixture should contain require fn");
}
...
fn test_split_rollup_fixture() {
    let source = std::fs::read_to_string("tests/fixtures/rollup_sample.js")
    ...
    assert!(split.files.len() >= 2, "Should produce at least 2 files: got {}", split.files.len());
    assert!(split.files.contains_key("index.js"), "Must produce index.js");
}

#[test]
fn test_split_webpack_fixture() {
    let source = std::fs::read_to_string("tests/fixtures/webpack_sample.js")
        .expect("fixture should exist");
    let pretty = unminify::pretty_print(&source, false).unwrap();
...
fn test_split_iife_deep_unwrap() {
    ...
    assert!(has_component, "Uppercase function should create a component file: {:?}", split.files.keys().collect::<Vec<_>>());
}
...
fn test_split_nonempty_files() {
    ...
    for (path, content) in &split.files {
        assert!(!content.trim().is_empty(), "File '{path}' should not be empty");
    }
}
